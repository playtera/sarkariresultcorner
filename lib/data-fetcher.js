...
async function loadSlugScrapedData(slug, dbEnabled) {
  ...
  try {
    ...
    const featuredImage = $('.wp-post-image').first().attr('src') ||
      $('.entry-content img').first().attr('src') ||
      $('.gb-container img').first().attr('src');
    ...
    if (gatheredHtml.length > 50) {
      ...
      // Remove SEO noise and unwanted blocks
      $content('script, ins, .adsbygoogle, .sharedaddy, .wp-block-buttons, a[href*="telegram"], a[href*="whatsapp"]').remove();
      $content('.yoast-text-optimization, .seo-meta-container, [class*="seo"], [id*="seo"]').remove();
      $content('.wp-block-latest-posts, .related-posts, .wp-block-latest-posts__list, .author-box, .breadcrumb').remove();
      $content('*').removeAttr('style');

      // Text-based removal for sections that don't have standard classes
      $content('h1, h2, h3, h4, h5, p, strong, div, td, th, a').each((_, el) => {
        ...
        if (unwantedKeywords.some(keyword => text.includes(keyword))) {
          // If in a table, remove the entire table
          const $table = $content(el).closest('table');
          if ($table.length > 0) {
            $table.remove();
          } else {
            const $parent = $content(el).closest('div, section, ul, aside, p');
      ...
      const postData = { title, htmlBody: finalHtml, featuredImage, isFromScraper: true };
